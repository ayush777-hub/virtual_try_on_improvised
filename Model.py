...
class Model(object):
    ...
    def predict(self, human_img, c_img, need_pre=True, need_bright=False, keep_back=False, need_dilate=False, check_dirty=False):
        ...
        if need_bright:
            ...
            human_img = np.array(enh_bri.enhance(1.2))
        ...
        if keep_back:
            ...
            if need_dilate:  # revise mask
                ...
                cloth = cloth_mask[:, :, 0]
                kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (18, 18))#18,18
                dilated = cv2.dilate(cloth, kernel)
                dilated = cv2.blur(dilated, (18, 18))#18,18
            ...
            out_img = human_img*(abs(0.98-cloth_mask))+out_img*cloth_mask
